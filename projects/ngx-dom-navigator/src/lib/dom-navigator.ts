import { DomNavigatorMode, DomNavigatorDirection } from './enums';
import { DomNavigatorSettings, DomNavigatorSelectedElement } from './interfaces';
...
export class DomNavigator {
	...
	private $selected: DomNavigatorSelectedElement;
	private $ignoredClassList: Array<string>;

	constructor(container: HTMLElement, settings: DomNavigatorSettings) {
		this.$doc = window.document;
		const childItems = Array.from(container.children);
		childItems.forEach((ch, i) => {
			ch.setAttribute('data-index', i.toString());
		});
	...
	private left() {
		...
		switch (this.settings.mode) {
			case DomNavigatorMode.AUTO:
				...
				next = this.elementsBefore(left, Infinity).reduce(
					(prev, curr) => {
		...
		this.select(next, DomNavigatorDirection.LEFT);
	}
	...
	private up() {
		...
		switch (this.settings.mode) {
			case DomNavigatorMode.AUTO:
				...
				next = this.elementsBefore(Infinity, top).reduce(
					(prev, curr) => {
						let currDistance = Math.abs(left - curr.offsetLeft) + Math.abs(top - curr.offsetTop);
						if (currDistance < prev.distance) {
							return {
		...
		this.select(next, DomNavigatorDirection.UP);
	}
	...
	private triggerClick() {
		...
		if (this.settings.onClick) {
			this.settings.onClick(this.$selected);
		}
	}
	...
	private right() {
		...
		switch (this.settings.mode) {
			case DomNavigatorMode.AUTO:
				...
				next = this.elementsAfter(left, 0).reduce(
					(prev, curr) => {
		...
		this.select(next, DomNavigatorDirection.RIGHT);
	}
	...
	private down() {
		...
		switch (this.settings.mode) {
			case DomNavigatorMode.AUTO:
				...
				next = this.elementsAfter(0, top).reduce(
					(prev, curr) => {
						let currDistance = Math.abs(curr.offsetLeft - left) + Math.abs(curr.offsetTop - top);
						if (currDistance < prev.distance) {
							return {
		...
		this.select(next, DomNavigatorDirection.DOWN);
	}
	 ...
	 */
	public get selected(): DomNavigatorSelectedElement {
	 ...
	 * @return void
	 */
	public select(el, direction: DomNavigatorDirection) {
		switch (direction) {
			case DomNavigatorDirection.DOWN:
				if (this.settings.onNavigateDown) {
					this.settings.onNavigateDown(el);
				}
				break;
			case DomNavigatorDirection.LEFT:
				if (this.settings.onNavigateLeft) {
					this.settings.onNavigateLeft(el);
				}
				break;
			case DomNavigatorDirection.RIGHT:
				if (this.settings.onNavigateRight) {
					this.settings.onNavigateRight(el);
				}
				break;
			case DomNavigatorDirection.UP:
				if (this.settings.onNavigateUp) {
					this.settings.onNavigateUp(el);
				}
				break;
		}
		...
		this.$selected.getIndex = () => Number(el.dataset.index);
	...
	private unSelectCurrent() {
		if (this.$selected) {
			...
			if (this.settings.onExit) {
				this.settings.onExit(this.$selected);
			}
		}
	}
			...
			switch (direction) {
				...
				case DomNavigatorDirection.RIGHT:
					this.$container.scrollLeft =
						...
						(this.$container.offsetWidth - el.offsetWidth);
					break;
				case DomNavigatorDirection.DOWN:
					this.$container.scrollTop =
						el.offsetTop -
						this.$container.offsetTop -
						(this.$container.offsetHeight - el.offsetHeight);
					break;
			}
	...
	private inContainerViewport(el) {
		...
		// Check on right side.
		if (
			el.offsetLeft + el.offsetWidth - this.$container.scrollLeft >
		...
		// Check on down side.
		if (
			el.offsetTop + el.offsetHeight - this.$container.scrollTop >
	...
	private elements() {
		...
		for (let i = this.$container.children.length; i--; ) {
			...
			if (
				this.$container.children[i].nodeType !== 8 &&
				!this.findCommonElement(this.$container.children[i].classList, this.$ignoredClassList)
			) {
